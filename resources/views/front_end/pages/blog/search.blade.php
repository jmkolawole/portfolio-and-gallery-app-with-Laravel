...
@section('content')
    ...
    <div class="wrapper light-wrapper">
        <div class="container inner pt-60">
            <h2 class="section-title mt-20 mb-60 text-center">Results  For "{{$search_term}}"</h2>
                            ...
                            <div class="row isotope">
                                ...
                                <?php
                                            ...
                                            if(isset($_GET['search'])){
                                             $post_term = $_GET['search'];
                                            $text = "<span style='color: deeppink'>";
                                            $text .= ucwords($post_term);
                                            $text .= "</span>";
                                            }
                                         ?>
                                    <div class="item post grid-sizer col-md-6 col-lg-6">
                                        <div class="box bg-white shadow p-30">
                                            ...
                                            <div class="meta mb-10"><span class="category"><a href="{{route('categories.blog',$post->category->slug)}}"
                                            class="hover color">{{$post->category->name}}</a></span></div>

                                            <h2 class="post-title">
                                                <a href="{{route('single',$post->slug)}}">
                                                    {!! str_replace(ucwords($post_term),$text,$post['title'])!!}

                                            </a></h2>
                                            <div class="post-content">
                                                <p>
                                                    <?php
                                                    $body = truncate(strip_tags($post->body), 100);
                                                    ?>
                                                        {!! str_replace(ucwords($post_term),$text,$body)!!}
                                                </p>
                                            </div>
                                            <!-- /.post-content -->
                                            <hr />
